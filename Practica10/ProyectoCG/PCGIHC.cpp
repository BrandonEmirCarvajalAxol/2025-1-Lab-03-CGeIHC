...
#define _CRT_SECURE_NO_WARNINGS //Arreglar un error en la apertura de archivos
...
FILE* fk = fopen("keyframesProyecto.txt", "w");
...
//NEW// Keyframes
float posXavion = 0.0, posYavion = -4.0, posZavion = 0.0;
...
void saveFrame(const char* archivo) //tecla L
{
	...
	fprintf(f, "KeyFrame[%d].movAvion_x = %.2f\n", FrameIndex, KeyFrame[FrameIndex].movAvion_x);
	fprintf(f, "KeyFrame[%d].movAvion_y = %.2f\n", FrameIndex, KeyFrame[FrameIndex].movAvion_y);
	fprintf(f, "KeyFrame[%d].movAvion_z = %.2f\n", FrameIndex, KeyFrame[FrameIndex].movAvion_z);
	fprintf(f, "KeyFrame[%d].giroAvion = %.2f\n", FrameIndex, KeyFrame[FrameIndex].giroAvion);
	fprintf(f, "\n\n");
	fclose(f);
	FrameIndex++;
}
...
int main()
{
	...
	*/
	...
	printf("3.-Presiona L para guardar frame\n4.-Presiona P para habilitar guardar nuevo frame\n5.-Presiona 1 para mover en X negativa\n6.-Presiona 2 para habilitar mover variables\n");
	printf("7.-Presiona 3 para mover en X positiva\n8.-Presiona 4 para mover en Y negativa\n9.-Presiona 5 para mover en Y positiva\n10.-Presiona 6 para girar 90 grados\n");
	printf("11.-Presiona 7 para girar menos 90 grados\n");
	printf("12.-Presiona 8 para mover en Z negativa\n");
	printf("13.-Presiona 9 para mover en Z positiva\n");

	glm::vec3 posCoolBus = glm::vec3(0.0f, -1.0f, 0.0f);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (mainWindow.getBandera()) //Presionar Z para mover el dado y X para reiniciar la tirada
		{
			if (movDadoy > 0.0 && movDadoX > -5.0)
			{
				...
				if (banderaDado == false) { // if para objener una sola vez por tirada de dados el valor de cada uno
					...
					avanzaDado = RD4+RD8; //variable para saber cuantas casillas avanzar 
		...
		model = glm::mat4(1.0);
		posCoolBus = glm::vec3(posXavion + movAvion_x, posYavion + movAvion_y, posZavion + movAvion_z);
		model = glm::translate(model, posCoolBus);
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_0])
	{
		if (habilitaranimacion < 1 && reproduciranimacion>0)
		{
			...
			habilitaranimacion++; //solo entro una vez al if
		}
	}
	...
	if (keys[GLFW_KEY_L])
	{
		if (guardoFrame < 1)
		{
			saveFrame("keyframesGuardados.txt");
