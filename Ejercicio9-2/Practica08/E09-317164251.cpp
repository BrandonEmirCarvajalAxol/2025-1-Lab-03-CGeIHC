	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (temp - aux >= 2.0)
		{
			if (baja)
			{
				if (movArco > -50.0f)
				{
					movArco -= movOffset * deltaTime;
				}
				else
				{
					rotaLetrero += movOffset2 * deltaTime;
					if (rotaLetrero >= 360.0) rotaLetrero = 0.0;
					if (temp - aux >= 2.0)
					{
						baja = !baja;
						aux = temp;
		...
		model = glm::translate(model, glm::vec3(20.0, -1.0f, 0.0f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = modelaux;
		model = glm::rotate(model, glm::radians(rotaLetrero), glm::vec3(0.0f, 1.0f, 0.0f));
